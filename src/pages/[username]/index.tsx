...
import { FIND_POST_BY_ID } from 'src/post/graphql-mutations'
const errorMessage =
...
const Profile = () => {
  ...
  const [getPostById, { data: findPost, loading: loadingByPost }] =
    useLazyQuery(FIND_POST_BY_ID)
  ...
  const handleSearchPost = (postId: string) => {
  ...
  //   let cleanup = true
  //   if (cleanup) {
  //     setAllPosts((prevPost) => [...prevPost, findPost])
  //   }
  //   return () => {
  //     cleanup = false
  //   }
    ...
    <>
      ...
      {editProfile ? (
        ...
        <>
              ...
              <div className="text-textGray flex flex-row flex-wrap">
                ...
                {data?.findProfile?.website && (
                  <Link href={`https://${data?.findProfile?.website}`}>
                    <a
                      target="_blank"
                      className="text-thirdBlue hover:underline"
                    >
                      {data?.findProfile?.website}
                    </a>
                  </Link>
          ...
          {/* <section>
            {data?.findProfile?.post.map((post: string) => (
              <article key={post}>{handleSearchPost(post)}</article>
            ))}
          </section> */}
        </>
      )}
